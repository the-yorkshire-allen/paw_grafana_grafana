...
if [ -d "$ANSIBLE_DIR/roles" ] && [ -f "$ANSIBLE_DIR/roles/loki/playbook.yml" ]; then
  # Collection structure
  PLAYBOOK_PATH="$ANSIBLE_DIR/roles/loki/playbook.yml"
  PLAYBOOK_DIR="$ANSIBLE_DIR/roles/loki"
  ...
  echo "{\"_error\": {\"msg\": \"playbook.yml not found in $ANSIBLE_DIR or $ANSIBLE_DIR/roles/loki\", \"kind\": \"puppet-ansible-converter/playbook-not-found\"}}"
  exit 1
fi
...
EXTRA_VARS="{"
FIRST=true
if [ -n "$PT_loki_target" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_target\": \"$PT_loki_target\""
fi
if [ -n "$PT_loki_auth_enabled" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_auth_enabled\": \"$PT_loki_auth_enabled\""
fi
if [ -n "$PT_loki_ballast_bytes" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_ballast_bytes\": \"$PT_loki_ballast_bytes\""
fi
if [ -n "$PT_loki_version" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_version\": \"$PT_loki_version\""
fi
if [ -n "$PT_loki_uninstall" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_uninstall\": \"$PT_loki_uninstall\""
fi
if [ -n "$PT_loki_http_listen_port" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_http_listen_port\": \"$PT_loki_http_listen_port\""
fi
if [ -n "$PT_loki_http_listen_address" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_http_listen_address\": \"$PT_loki_http_listen_address\""
fi
if [ -n "$PT_loki_expose_port" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_expose_port\": \"$PT_loki_expose_port\""
fi
if [ -n "$PT_loki_download_url_rpm" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_download_url_rpm\": \"$PT_loki_download_url_rpm\""
fi
if [ -n "$PT_loki_download_url_deb" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_download_url_deb\": \"$PT_loki_download_url_deb\""
fi
if [ -n "$PT_loki_working_path" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_working_path\": \"$PT_loki_working_path\""
fi
if [ -n "$PT_loki_ruler_alert_path" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_ruler_alert_path\": \"$PT_loki_ruler_alert_path\""
fi
if [ -n "$PT_loki_server" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_server\": \"$PT_loki_server\""
fi
if [ -n "$PT_loki_common" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_common\": \"$PT_loki_common\""
fi
if [ -n "$PT_loki_query_range" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_query_range\": \"$PT_loki_query_range\""
fi
if [ -n "$PT_loki_schema_config" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_schema_config\": \"$PT_loki_schema_config\""
fi
if [ -n "$PT_loki_ruler" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_ruler\": \"$PT_loki_ruler\""
fi
if [ -n "$PT_loki_analytics" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"loki_analytics\": \"$PT_loki_analytics\""
fi
EXTRA_VARS="$EXTRA_VARS}"
